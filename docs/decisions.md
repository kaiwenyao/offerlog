...
## ADR-005 桑基图口径
- 当前进度桑基 A：样本=未删除未归档申请；未投递是叶子；已投递即等待反馈，后续只展开真正走过的笔试 / 初筛 / 面试 / Offer，终态挂在链尾。停留人数是节点入流减出流，不另拆叶子。
...
- 2026-09 更新（口径版本 v3）：当前进度不再把已投递一次性按当前状态摊平。
  已投递节点本身就是等待反馈（applied / 投递后退回准备材料都停在这里，不另开列）。
  之后只展开 `application_stage_points` 里已经发生的后段（笔试 → 初筛 → 面试 → Offer，
  只保留真正出现过的），终态接到链尾。于是「笔试后被拒」会经过「笔试作业」再到「被拒绝」。
  仍停在某阶段的人数留在该节点上（入流大于出流），不拆「当前：…」叶子。
