...
//
// 两个字段都是可选的：nil 表示「这次不改」，由 COALESCE 保留库里的值，并把提交前
// 的当前行回读出来。PUT /preferences 只带提醒开关时必须走这条路——它手上的 user
// 是进事务之前的会话快照，把 display_name 无条件写回去就会把同时保存的改名悄悄
// 回滚（两个请求各自 PUT 一部分字段，后一个的快照还是旧名字）。
func (r *SQLUsers) UpdateProfileTx(ctx context.Context, q database.Querier, id int64, displayName, timezone *string) (*auth.UserRow, error) {
	var u auth.UserRow
	err := q.QueryRow(ctx, `UPDATE users SET display_name=COALESCE($2, display_name),
